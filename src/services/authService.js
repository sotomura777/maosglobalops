import { createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut as fbSignOut, onIdTokenChanged } from 'firebase/auth';
...
export async function signUpWorker(email, password, name) {
  ...
  try {
    await setDoc(doc(db, 'profiles', cred.user.uid), {
      kind: 'worker', name, email: cred.user.email, gdprConsent: true, createdAt: new Date().toISOString(),
    });
  } catch (e) {
